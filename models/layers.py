...
class Conv2d_flip(nn.Conv2d):

    def __init__(self, in_channels, out_channels, kernel_size, stride=1,
                 padding=0, dilation=1, groups=1, bias=True):
        super(Conv2d_flip, self).__init__(in_channels, out_channels, kernel_size, stride,
                                     padding, dilation, groups, bias)
...
class Conv2d_new(nn.Conv2d):
    ...
    def forward(self, x):  # return super(Conv2d, self).forward(x)
        ...
        return F.conv2d(x, (1/(1+torch.abs(self.alpha)))*weight /torch.sqrt(self.moving_var + eps), \
